...
export function renderEventText(code: string, p: Params): string {
  switch (code) {
    ...
    case "contributionRule.added":
      return `Beitragsregel hinzugefügt (${String(p["ruleType"])}, ${euro(p["amountMinor"] as number | undefined)})`;
    case "contributionRule.updated":
      return `Beitragsregel aktualisiert (${String(p["ruleType"])}, ${euro(p["amountMinor"] as number | undefined)})`;
    case "contributionRule.removed":
      return `Beitragsregel entfernt (${String(p["ruleType"])})`;
    case "memberIncome.added":
      return `Einkommen hinzugefügt (${euro(p["amountMinor"] as number | undefined)})`;
    case "memberIncome.updated":
      return `Einkommen aktualisiert (${euro(p["amountMinor"] as number | undefined)})`;
    case "memberIncome.removed":
      return "Einkommen entfernt";
    case "categoryBudget.added":
      return `Kategorie-Budget gesetzt (${euro(p["amountMinor"] as number | undefined)})`;
    case "categoryBudget.updated":
      return `Kategorie-Budget aktualisiert (${euro(p["amountMinor"] as number | undefined)})`;
...
function txLabel(prefix: string, p: Params): string {
  ...
  const amount = euro(p["amountMinor"] as number | undefined);
